...
BEGIN
  ...
  IF authentication_method IN ('oauth_provider/authorization_code', 'token_refresh')
     AND event ? 'client_id' AND claims ? 'client_id'
  ...
  -- Supabase supplies the authenticated user as event.user_id and JWT sub.
  -- OAuth access-token claims do not contain a separate user_id claim.
...
COMMENT ON FUNCTION public.normic_custom_access_token_hook(JSONB) IS
  'Supabase Custom Access Token Hook. Binds the trusted event user_id to JWT sub, supports dynamic clients through server-controlled grants, and preserves unrelated sessions.';
