...
Wallet preparation: call `normic_prepare_wallet` with a UUID `idempotencyKey` using an authenticated MCP credential with `company:read` and a live owner OAuth grant. The company is resolved from that credential, never from agent-supplied owner identifiers. An existing wallet returns `READY`; otherwise `OWNER_APPROVAL_REQUIRED` includes a 10-minute `https://normic.tech/wallet` review link. Reuse the same key for retries; request a new link with a new key after expiry. The link is not a bearer grant: its owner must sign in, click Create Normic Wallet, and personally complete the existing passkey ceremony. Requests reuse the financial idempotency journal and immutable root reservation; no migration is needed. Query `normic_get_wallet` with `{}` afterward (`companyId` remains optional for existing clients): it returns `READY` with the public address or `NOT_CREATED`. No passkey challenge is delivered through MCP, no wallet is provisioned by the agent, and no spending/session authority or transaction is created.
